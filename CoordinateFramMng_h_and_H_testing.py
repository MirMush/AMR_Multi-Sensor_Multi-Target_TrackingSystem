...
gt_data = data['ground_truth']#gets the ground truth data for the vessel (pN, pE, vN, vE) at each timestamp,[0] is the target ID
...
for m in measurements[:100]: # Limit to first 100 entries for a clean output
    t = m['time'] #Pulls the timestamp out of the current measurement.
    s_id = m['sensor_id']#Pulls the sensor ID out of the current measurement.
